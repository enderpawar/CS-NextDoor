...
> PWA는 **독립 모드** (PC 부팅 불가 → Electron 없이 직접 접속)와 **세션 모드** (Electron QR 연결) 두 가지로 동작합니다. !!수정사항 : PWA 환경에서도 별도 세션을 생성하여 작업을 진행할 수 있도록 수정 
