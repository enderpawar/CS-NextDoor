...
| **7-C** | PWA | 갤러리 업로드 진단 — 실시간 촬영 불가 시 갤러리 이미지/영상으로 대체 진단 | `GalleryUpload.tsx` |
...
| **7** | ✅ 완료 — useOpenCV(2단계 WASM 초기화), CameraView(processFrame export + canvas rAF 오버레이), VideoAnalysis(15프레임→상위5개 선별 + iOS audio 폴백), ShootingGuide(bottom-sheet 다이어그램). frames 실제 전달. USE_MOCK=true. 빌드·58테스트 통과 |
| **7-C** | 🔲 미시작 — 갤러리 업로드 진단 (실시간 촬영 불가 사용자 대상 대체 입력 경로) |
| **7-B** | 🔲 미시작 |
| **8** | 🔲 미시작 |
| **9** | 🔲 미시작 |
| **10** | 🔲 미시작 |
