...
| **7** | ✅ 완료 — useOpenCV(2단계 WASM 초기화), CameraView(processFrame export + canvas rAF 오버레이), VideoAnalysis(15프레임→상위5개 선별 + iOS audio 폴백), ShootingGuide(bottom-sheet 다이어그램). frames 실제 전달. USE_MOCK=true. 빌드·54테스트 통과 |
| **7-B** | 🔲 미시작 |
| **8** | 🔲 미시작 |
| **9** | 🔲 미시작 |
| **10** | 🔲 미시작 |
